...
Brain.prototype._handleScan = function _handleScan (address) {
  ...
  if (this.currentCoin == 'BTC' && !(bitcoinAddressValidator.validate(address))) {
    // looks like an invalid bitcoin address
    this._restart();
  } else if (this.currentCoin == 'DASH' && !(dashAddressValidator.validate(address))) {
    // looks like an invalid dash address
    this._restart()
  } else {
