...
Brain.prototype._idle = function _idle (locale) {
  ...
  this._chooseCoin(localeInfo)
}

Brain.prototype._post_idle = function _post_idle (coin) {
  var self = this

  this.trader.setCoin(this.currentCoin, function(err, res) {
    if (err) {
      self._restart()
    } else {
      console.log("set coin result: " + JSON.stringify(res));
      // We've got our first contact with server
      if (self.trader.twoWayMode && self.cartridges()) {
        self._ensureRaqiaRegistration(function () {
          self._idleTwoWay()
        })
      } else {
        self._idleOneWay()
...
Brain.prototype._idleTwoWay = function _idleTwoWay () {
  ...
  this.billDispenser.init({
    ...
    self._transitionState('dualIdle',
      {cartridges: uiCartridges})
  })
}

Brain.prototype._idleOneWay = function _idleOneWay () {
  this._transitionState("post_idle")
}

Brain.prototype._chooseCoin = function _chooseCoin (localeInfo) {
...
Brain.prototype._handleScan = function _handleScan (address) {
  ...
  // TODO use real address validation
  if (this.currentCoin == 'BTC' && !(this.coinAddress.lastIndexOf('1', 0) === 0 || this.coinAddress.lastIndexOf('3', 0) === 0)) {
    // looks like an invalid bitcoin address
    this._restart();
  } else if (this.currentCoin == 'DASH' && !(this.coinAddress.lastIndexOf('2', 0) === 0 || this.coinAddress.lastIndexOf('X', 0) === 0)) {
    // TODO is '2' a valid first digit for a dash addrss?
    this._restart()
  } else {
...
Brain.prototype._firstBill = function _firstBill () {
  ...
  this._logTx({sessionId: this.trader.sessionId, coinAddress: address},
    'scanAddress')
}
...
Brain.prototype._billValid = function _billValid () {
  ...
  tradeRec.toAddress = this.coinAddress
...
Brain.prototype._cashOut = function _cashOut () {
  ...
  this.trader.cashOut(tx, function (err, coinAddress) {
    if (err) return self._fiatError()
    tx.toAddress = coinAddress
