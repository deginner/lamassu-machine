...
Brain.prototype._start = function _start () {
  ...
  var fiatBalance = this.fiatBalance()
  var highestBill = this.getBillValidator().highestBill(fiatBalance)
  // TODO isysd disabled for testing
...
Brain.prototype.isStaticState = function isStaticState () {
  var staticStates = ['chooseCoin', 'idle', 'pendingIdle', 'dualIdle',
    'networkDown', 'maintenance', 'virgin', 'wifiList']
...
Brain.prototype.fiatBalance = function fiatBalance () {
  var cryptoCode = this.cryptoCode
  if (!cryptoCode) throw new Error('No cryptoCode, this shouldn\'t happen')
  return this.trader.rates(cryptoCode).cashIn.mul(this.trader.balances[cryptoCode])
}
...
Brain.prototype.sendRates = function sendRates () {
  ...
  var rec = {
    currency: this.currency,
    rates: {
      rates: {cashIn: this.trader.rates(cryptoCode).cashIn.toFixed(15),
              cashOut: this.trader.rates(cryptoCode).cashOut.toFixed(15)},
      cryptoCode: cryptoCode,
      coins: coins
    },
    coins: this.trader.coins,
...
Brain.prototype._billRead = function _billRead (data) {
  ...
  var fiatBalance = this.fiatBalance() - totalFiat
  ...
  if (txLimit && totalFiat > txLimit || fiatBalance < 0) {
    ...
    var newFiatBalance = this.fiatBalance() - this.credit.fiat
