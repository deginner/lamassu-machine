...
function scanMainQR (callback) {
  scanQR(function (err, result) {
    ...
    if (resstr.indexOf("bitcoin:") > -1) {
      callback(null, processBitcoinURI(resstr));
    } else if (resstr.indexOf("dash:") > -1) {
      callback(null, processDashURI(resstr));
    } else if (resstr.indexOf("1") == 0 || reststr.indexOf("3") == 0) {
      callback(null, processBitcoinURI(resstr));
    } else if (resstr.indexOf("X") == 0) {
      callback(null, processDashURI(resstr));
    } else {
      callback("unable to scan address", null);
    }
  })
}
