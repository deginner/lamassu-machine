...
function scan (mode, resultCallback, captureCallback) {
  ...
  function capture () {
    ...
    cam.capture(function (success) {
      if (!success) return
      var buffers = [];
      var rgb = img.yuyv2rgb(cam.toYUYV(), cam.width, cam.height);
      var png = img.asPng(rgb, cam.width, cam.height);
      var str = png.pack().pipe(base64.encode())
      str.on('data', function(buffer) {
          buffers.push(buffer);
      });
      str.on('end', function() {
          var buffer = Buffer.concat(buffers);
          captureCallback(null, 'data:image/png;base64,' + buffer.toString(), function(err, result) {
              if (result) {
                  clearInterval(handle)
                  cam.stop()
